...
class PropertiesBase(BaseModel):
    ...
    logp: Optional[float] = Field(
        None,
        description="LogP (octanol-water partition coefficient) from RDKit",
    ...
    tpsa: Optional[float] = Field(
        None,
        ge=0.0,
        description="Topological polar surface area (Å²) from RDKit",
        json_schema_extra={"example": 456.78},
    )

    num_h_donors: Optional[int] = Field(
        None,
        ge=0,
    ...
    num_h_acceptors: Optional[int] = Field(
        None,
        ge=0,
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        ...
        json_schema_extra={
            "example": {"calculation_version": "1.79", "calculation_date": "2024-01-24"}
        },
    )
...
class PhysicochemicalProperties(PropertiesBase):
    ...
    id: UUID = Field(..., description="Unique identifier for the properties record")
...
class PropertiesWithPeptide(PhysicochemicalProperties):
    ...
    peptide_name: Optional[str] = Field(None, description="Name of the peptide")

    peptide_sequence: Optional[str] = Field(None, description="Sequence of the peptide")

    sequence_length: Optional[int] = Field(
        None, ge=0, description="Length of the peptide sequence"
    )
